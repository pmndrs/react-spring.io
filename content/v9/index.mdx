...
Behold! Every feature introduced in v9, on this page!

Want to know the **breaking changes**? 👻 [Go here!](/v9/breaking-changes)
...
Read the [Breaking Changes](/v9/breaking-changes) page to learn about the new
API for the `useTransition` hook.
...
Notice how the loop doesn't run more than once. That's because some props
...
Before we move on, try overriding the inherited `config` prop by adding
`config: { friction: 5 }` to the `loop` object in the example above.
...
diffing algorithm within [`SpringValue#_merge`][merge1].
...
Any prop whose name starts with `on` is affected.
...
The `onProps` prop is called for every update that wasn't prevented.
It receives a props object and the affected `SpringValue` object.

It can be useful when debugging an animation.

This prop is called immediately after all props have been merged into
the `animation` object owned by the `SpringValue` passed to you.

This prop is called before the first frame of the updated animation.
