...
### Changes in `config.velocity`

The calculation with `velocity` in `config` has been changed. Coming from v8,
you need to multiply the value of `velocity` with `0.001` to keep the same
behavior.

So in v8 if the config was
...
then it needs to be changed into
