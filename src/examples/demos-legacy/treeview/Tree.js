...
export default class Tree extends React.PureComponent {
  ...
  toggleVisibility = () => {
    this.setState(state => ({visible: !state.visible, immediate: true}), () => this.props.onClick && this.props.onClick(this.state.visible))
  }
  ...
  render() {
    ...
    return (
      <div style={{...styles.tree, ...style}} className="treeview">
        <Icon className="toggle" style={{...styles.toggle, opacity: children ? 1 : 0.3}} onClick={this.toggle} />
        <span style={{...styles.type, marginRight: type ? 10 : 0}}>{type}</span>
        {canHide && <Icons.EyeO className="toggle" style={{...styles.toggle, opacity: visible ? 1 : 0.4}} onClick={this.toggleVisibility} />}
        ...
        >
          {style => <animated.div style={{...style, ...styles.contents}}>{children}</animated.div>}
        </Spring>
      </div>
    )
  }
