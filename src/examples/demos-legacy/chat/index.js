...
function addItem(state) {
  ...
  const text = lorem() + (Math.round(Math.random()) ? '' : ` ${emoji.random({count: 1})}`)
...
export default class App extends React.PureComponent {
  ...
  addItems = () => setTimeout(() => void (this.setState(addItem), count++ < 10 && this.addItems()), this.state.items.length ? Math.random() * 1000 : 0)
        ...
        <ul className="ul-c" ref={this.list}>
          ...
          >
            {({text, left, first, last}, i, state) => styles => (
              <animated.li ref={state === 'enter' && this.el} className={classnames({left, right: !left, first, last})} style={styles}>
                {text}
              </animated.li>
            )}
          </Transition>
